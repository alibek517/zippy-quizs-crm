...
function ResultPage() {
  ...
  return (
    <div className="max-w-3xl space-y-4">
      <div>
  <h1 className="text-3xl font-bold text-black">Natija</h1>
        <p className="text-muted-foreground">{(data as any).tests?.title}</p>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
  <Card className="p-4"><div className="text-xs text-muted-foreground">Foiz</div><div className="text-2xl font-bold" style={{ background: 'var(--gradient-gold)', WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent' }}>{data.score_percent}%</div></Card>
